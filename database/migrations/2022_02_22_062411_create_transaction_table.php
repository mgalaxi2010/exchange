...
class CreateTransactionTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('transactions', function (Blueprint $table) {
            ...
            $table->decimal('price_from',38,18)->unsigned();
            $table->decimal('amount_from',38,18)->unsigned();
            $table->integer('coin_id_to')->unsigned();
            $table->foreign('coin_id_to')->references('id')->on('coins')->onDelete('cascade');
            $table->decimal('price_to',38,18)->unsigned();
            $table->decimal('amount_to',38,18)->unsigned();
            $table->timestamps();
        });
    }
